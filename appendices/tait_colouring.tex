\label{sec:tait_colourings}
In this appendix we will discuss a very old theorem first proposed by scottish physicist Peter Tait, who in the late 1800s was working towards a proof of the four colour theorem. In his attempt to simplify the problem, Tait found a correspondence between the problem of four-colouring the faces of a graph and three-colouring the edges \cite{Tait1880, appelEveryPlanarMap1989a}. In fact, Tait used this result to construct an incorrect proof of the four-colour theorem, which stood unchallenged for 11 years until a counterexample was provided by Julius Petersen \cite{peterson_theorie_1891,mulder_julius_1992}. Here we will provide a simple proof of Tait's original correspondence, which states that:
...
\begin{center}
    \includegraphics{appendices/diagrams/bridge.pdf}
\end{center}
...
\end{align}
although it does fail to satisfy some of the other properties one might expect of subtraction such as having $\mathfrak l_1 \oplus \mathfrak l_2 = \mathfrak l_2 \oplus \mathfrak l_1$.
...
Now let us suppose that we have two labelled faces, $f_1$ and $f_2$, which are separated by an edge $e$. We assert that the a valid edge colouring can be found by taking the difference of the two adjacent face colours
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{appendices/diagrams/four_colouring.pdf}
    \caption{ An example of a 3-edge colouring on a graph with coordination number $z\leq3$, along with the corresponding 4-face colouring, related according to eqn.~\ref{eqn:face_to_edge}. }
...
\begin{center}
    \includegraphics{appendices/diagrams/site_e_f.pdf}
\end{center}
...
\begin{figure}[t]
    \centering
    \includegraphics{appendices/diagrams/f_paths.pdf}
    \caption{... }
    \label{fig:two_paths}
\end{figure}
\begin{align}
...
\begin{align}
    \mathfrak f_1 \otimes e = f_2 \neq f_1 \textup{ for } e \neq \textbf 0.
\end{align}

\begin{shaded}
    {\it What about periodic boundaries?} ---
    The eagle-eyed reader might have noticed an omission in the argument presented. We showed that all closed loops that encircle a set of vertices are guaranteed to have $\bigotimes e = \textbf 0$. However not all closed loops have a defined inside and outside. If we construct such a system on a manifold with genus $g$, then in general there will be $2g$ topologically distinct non-contractible loops. Since these have no inside and outside, the argument above fails. 
